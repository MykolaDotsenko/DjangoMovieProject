...
class EnvironmentParsingTests(SimpleTestCase):
    ...
    def test_env_bool_rejects_unknown_values(self):
        with patch.dict(os.environ, {"FLAG": "maybe"}):
            with self.assertRaisesMessage(ImproperlyConfigured, "FLAG must be one of"):
                env_bool("FLAG")
    ...
    def test_env_int_parses_values_and_enforces_minimum(self):
        ...
        with patch.dict(os.environ, {"PORT": "0"}):
            with self.assertRaisesMessage(ImproperlyConfigured, "PORT must be at least 1"):
                env_int("PORT", 5432, minimum=1)

    def test_env_int_rejects_non_integer_values(self):
        with patch.dict(os.environ, {"PORT": "five"}):
            with self.assertRaisesMessage(ImproperlyConfigured, "PORT must be an integer"):
                env_int("PORT", 5432)
    ...
    def test_required_env_rejects_missing_value(self):
        with patch.dict(os.environ, {}, clear=True):
            with self.assertRaisesMessage(ImproperlyConfigured, "TOKEN must be set"):
                required_env("TOKEN")
